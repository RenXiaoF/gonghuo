// pages/ship_mana_detail/ship_mana_detail.js
//获取应用实例
const app = getApp();
...
Page({
  ...
  onShow: function (options) {
    ...
    that.setData({gotoRouter: options.gotoRouter});
    if (options.gotoRouter == "0") {
    ...
    if (options.gotoRouter == "1") {
      // 动态修改页面标题
      wx.setNavigationBarTitle({
        title: "已发货详情",
      });
    }
    if (options.gotoRouter == "2") {
      // 动态修改页面标题
      wx.setNavigationBarTitle({
        title: "发货详情",
      });
